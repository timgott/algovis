...
import { findAllInjectiveMatchesGeneric } from '../../subgraph/src/matching';
...
describe("test makeRuleFromOperatorGraph", () => {
    ...
    test("negative edges subgraph", () => {
        ...
        expect([...findAllInjectiveMatchesGeneric(graph.nodes, pattern.nodes, matcher)]).toHaveLength(0)
    })
    ...
    test("negative edges rule", () => {
        ...
        expect([...findAllRuleMatches(graph, rule)]).toHaveLength(1)

        // now test negative edge
        createEdge(graph, a2, c2)
        expect([...findAllRuleMatches(graph, rule)]).toHaveLength(0)
    })
})